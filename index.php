			...
			<div class="divBorder" style="min-height: 470px;">
				...
				<table style="width: 100%">
					<tr class="select-shipping" style="display: none;">
						<td style="vertical-align: top;">
							<h4>Ship to:</h4>
							<address>
								<strong><span id="addr-recipient"></span></strong><br /> <span
									id="addr-line1"></span><br /> <span id="addr-line2"></span> <span
									id="addr-city"></span> <span id="addr-state"></span> <span
									id="addr-postal-code"></span><br /> <span
									id="addr-country-code"></span>
							</address>

						</td>
						<td style="vertical-align: top;">
							<h4>Select&nbsp;shipping rate:</h4> <select
							class="required-entry form-control" name="shipping-method"
							id="shipping-method" onChange="updateTotalAmount(this) ">
				...
				<table class="table table-striped">
					...
					<tr class="select-shipping" style="display: none;">
						...
						<td><span id="total-amount-display"></span></td>
					</tr>

				</table>

				<div style="text-align: center;" id="initial-message">
					<span style="font-weight: bold;">Mouse-over the buttons to review
						the Payment Flow diagrams.</span>
				</div>
				...
				<div id="pay-now" style="display: none; text-align: center;">
					<form action="bt/bt-transaction.php" method="post">
						<input type="hidden" name="checkout" value="classic" /> <input
							type="hidden" name="payment-method-nonce"
							class="payment-method-nonce" /> <input type="hidden"
							name="item-quantity" value="1" /> <input type="hidden"
							name="item-description" value="DSLR Camera, Auto-focus, A0123" />
							<input type="hidden" id="currencyBT" name="currency" value= "USD" /> <input
							type="hidden" id="first-name" name="first-name" value="" /> <input
							type="hidden" id="last-name" name="last-name" value="" /> <input
							type="hidden" id="recipient" name="recipient" value="" /> <input
							type="hidden" id="line1" name="line1" value="" /> <input
							type="hidden" id="line2" name="line2" value="" /> <input
							type="hidden" id="city" name="city" value="" /> <input
							type="hidden" id="state" name="state" value="" /> <input
							type="hidden" id="postal-code" name="postal-code" value="" /> <input
							type="hidden" id="country-code" name="country-code" value="" /> <input
							type="hidden" id="item-amount" name="item-amount" value="300.00" />
						<input type="hidden" id="total-amount" name="total-amount"
							value="" />

						<button type="submit" style="width: 226px;" id="pay-now"
							type="button" class="btn btn-primary" onClick="helper();">Pay Now</button>
					</form>
				</div>
				...
				<div style="text-align: center;" id="second-flow">
					...
					<form action="bt/checkout.php" method="post">
						<input type="hidden" name="item-name" value="DSLR Camera" /> <input
							type="hidden" name="item-id" value="A0123" /> <input
							type="hidden" name="item-description" value="Autofocus" /> <input
							type="hidden" name="item-quantity" value="1" /> <input
							type="hidden" name="item-price" value="300.00" /> <input
							type="submit" value="Proceed to Checkout"
							class="btn btn-primary btn-large" id="continue" />
					</form>
				</div>
			</div>
		</div>
...
<script>
			...
			*/		
			braintree.client.create({
				...
				authorization: '<?php require_once('bt/bt-get-client-token.php') ?>'
				...
				braintree.paypalCheckout.create({
					...
					paypal.Button.render({
						...
						onCancel: function (data) {
							...
							var cancelUrl = currentUrl.substring(0, currentUrl.lastIndexOf('/')) + '/include/cancel.php';
